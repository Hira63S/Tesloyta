...
def compute_deltas(boxes_xyxy, anchors_xywh):
    ...
    for i in range(boxes_xyxy.shape[0]):
        # for all the boxes in the boxes_xyxy, we compute the overlaps between
        # each other
        ...
        # if the largest available overlap is 0, choose the anhor box with the one that has the
...
def boxes_postprocess(boxes, image_image):
    ...
    if 'flipped' in image_meta and image_meta['flipped']:
        ...
        boxes[:, 0] = boxes_width - 1 - boxes[:, 2]
        boxes[:, 2] = boxes[:, 0] + boxes_width - 1.
