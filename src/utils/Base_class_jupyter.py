...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def preprocess(self, input_size, image, image_meta, boxes=None, drift_prob, flip_prob, forbid_resize=False):
        """
        ...
        if forbid_resize == True:
            image, image_meta, boxes = crop_or_pad(image, image_meta, input_size, boxes=boxes)
        else:
            image, image_meta, boxes = resize(image, image_meta, input_size, boxes=boxes)

        return image, image_meta, boxes

    def prepare_annotations(self, class_ids, boxes, anchors, num_anchors, num_classes):
        """
        :param class_size
        :param boxes: xyxy format
        :return: np.ndarray(#anchors, #classes + 9)
        """
        deltas, anchor_indices = compute_deltas(boxes, anchors)

        gt = np.zeros((num_anchors, num_classes + 9), dtype=np.float32)
