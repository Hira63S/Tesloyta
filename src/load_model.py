...
def load_model(model, model_path):
    ...
    print('loaded model {}, epoc {}'.format(model_path, checkpoint['epoch']))
    ...
    for layer in model_state_dict:
        if layer not in state_dict:
            success_loaded = False
            print('Param {} not found in the pre-trained model'.format(layer))
            state_dict[layer] = model_state_dict[layer]
...
def load_official_model(model, model_path):
    """
    load official models from the torchhub
    :param model:
    :param model_path:
    :return:
    """
...
def save_model(model, path, epoch):
    if isinstance(model, torch.nn.DataParrallel):
...
print('Hello Tesla! I am loaded model.')
